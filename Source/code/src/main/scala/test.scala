...
object test {
  def main(args: Array[String]): Unit = {
    ...
    val input = sc.textFile("C:\\Users\\user\\Desktop\\bbc\\entertainment\\013.txt").map(line => line.split(" ").toSeq)

    val modelFolder = new File("testset")

    if (modelFolder.exists()) {
      val sameModel = Word2VecModel.load(sc, "testset")
      val synonyms = sameModel.findSynonyms("politics", 40)
